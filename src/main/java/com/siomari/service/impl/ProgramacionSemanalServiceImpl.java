...
/// **
// * se haran la suma de las areas de los canales de distribucion de la unidad
/// en
// * la programacion semanal solicitada (fecha, la cual debe ser un lunes)
// * eficiencia (promedio con la informacion del manejo agua de hace una semana
/// de
// * la fecha especificada)
// *
// * @param id
// * de la unidad
// * @param fecha
// * fecha en la que comenzara a regir la programacion (debe de ser un
// * lunes)
// * @return area, eficiencia, caudal, id sera -1 si no se ha especificado el
/// canal servidor, -2 si
// * no se ha especificado los canales servidores de las zonas
// */
// private ProgramacionSemanal calculoCaudalUnidad(int id, LocalDate fecha) {
//
// int idCanal = unidadService.buscarCanalServidorPorId(id);
//
// /*
// * Si el id es 0 singnifica que en la unidad no han especificado el canal
/// servidor
// */
// if(idCanal == 0) return new ProgramacionSemanal(-1);
//
// // verificamos si la informacion ya ha sido registrada
// ProgramacionSemanal programacionSemanal = this.buscarPorFechaYCanalId(fecha,
/// idCanal);
//
// if(programacionSemanal != null) return programacionSemanal;
//
// // traemos los id de los canales servidores de las zonas de la unidad
// List<Integer> lstServidores = zonaService.buscarCanalServidor(id);
//
// // sera false si todos los canales servidores de las zonas son nulas
// boolean zonasServidores = false;
//
// // sumatoria de las areas de los canales servidores de la unidad
// double area = 0;
// // sumaremos el caudal de los canales servidores para multiplicarlo por el
/// factor de la unidad
// double sumCaudal = 0;
...
//
// // especificamos que si habian canales servidores registrados en las zonas
// zonasServidores = true;
//
// // traemos la programacion semanal del canal que ya se debio haber retistrado
// ProgramacionSemanal data = this.buscarPorFechaYCanalId(fecha, idServidor);
//
...
// }
//
// // no hacemos los calculos si no hay canales servidores especificados en las
/// zonas
// if(!zonasServidores) return new ProgramacionSemanal(-2);
//
// double eficiencia = this.calculoEficiencia(idCanal, fecha);
//
// double caudal = 0;
//
// // si la eficiencia es cero solo deolvemos el caudal que se calculo
// if(eficiencia == 0) {
// caudal = sumCaudal;
// } else {
// caudal = sumCaudal * Math.pow(eficiencia, -1);
// }
//
// programacionSemanal = new ProgramacionSemanal();
//
// programacionSemanal.setArea(area);
// programacionSemanal.setCaudal((double) Math.round(caudal * 1000d) / 1000d);
// programacionSemanal.setEficiencia(eficiencia);
//
//
// return programacionSemanal;
